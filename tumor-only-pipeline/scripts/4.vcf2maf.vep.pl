...
## The vcf2maf.pl copys from Song's somaticwrapper pipeline v1.5
# Hua modified 09/13/2019
# Usage
# vcf2maf.pl --input-vcf <original.vcf> --output-maf <out.maf> --tumor-id <tumor.name> --normal-id <normal.name> --ref-fasta <genome_ref.fa>
# or
# vcf2maf.pl --input-vcf <original.vcf> --output-maf <out.maf> --tumor-id <tumor.name>
...
# Set VEP program & VEP database direction
my $VEP = '/gscmnt/gc2525/dinglab/rmashl/Software/bin/VEP/v85/ensembl-tools-release-85/scripts/variant_effect_predictor';
my $VEP_DIR = '/gscmnt/gc2518/dinglab/scao/tools/vep/v85';
my ( $vep_path, $vep_data, $vep_forks ) = ( "$VEP", "$VEP_DIR", 4 );


# Set TSL file location
my $file_tsl = '/gscmnt/gc2518/dinglab/scao/db/tsl/wgEncodeGencodeTranscriptionSupportLevelV23.txt'; 

# Default:
# $ncbi_build is GRCh38
# $cache_version is '85'
# $ref_fasta is 'GRCh38'


my $ref_fasta = '/gscmnt/gc2737/ding/hsun/data/human_genome/gencode_GRCh38_v29/genome/GRCh38.primary_assembly.genome.fa';
my $filter_vcf = '';
...
my ( $species, $ncbi_build, $cache_version, $maf_center, $retain_info, $min_hom_vaf, $max_filter_ac ) = ( "homo_sapiens", "GRCh38", "85", ".", "", 0.7, 16 );
...
if( $ncbi_build eq "GRCh37" or ( $filter_vcf and $filter_vcf ne "$ENV{HOME}/.vep/ExAC_nonTCGA.r0.3.1.sites.vep.vcf.gz" )) {
    ...
    foreach my $line ( split( "\n", $lines )) {
        ...
        my $locus = ( $chr_prefix_in_use ? "$chr:$pos" : "chr$chr:$pos" );
...
sub FixAlleleDepths {
    ...
    # Handle VCF lines by Mutect2 GT:AD:AF:DP:F1R2:F2R1:SB
    elsif( defined $fmt_info{AD} ) {
        @depths = map{""} @alleles;
        my ($ref_depth, $alt_depth) = split(/,/, $fmt_info{AD});
        $depths[0] = $ref_depth;
        $depths[$var_allele_idx] = $alt_depth;
    }
